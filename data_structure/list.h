...
int main(int argc, char **argv)
{
	...
	// list_add
	for (elem = students; elem->age != 0; elem++) {
		list_add(&elem->entry, &head.entry);
	}

	// list_for_each
	list_for_each(pos, &head.entry) {
		s = list_entry(pos, struct student, entry);
		printf("name:%s age:%d\n",
			   s->name, s->age);
	}
	...
	INIT_LIST_HEAD(&ano_head.entry);
	struct student one = {34, "Tom"};
	list_add(&one.entry, &ano_head.entry);
	struct student two = {47, "Jerry"};
	list_add(&two.entry, &ano_head.entry);

	printf("\n");

	 // list_splice
	list_splice(&ano_head.entry, &head.entry);
