...
VOSConfig::VOSConfig() : m_iLoaded(0), m_nullRet("")
{	
}

int VOSConfig::loaded()
{
	return m_iLoaded;
}

VOSConfig * VOSConfig::instance()
{
	static VOSConfig * _ins  = 0;
...
int VOSConfig::loadConfigFile(const  char * s_cnf_file)
{
	if(NULL == s_cnf_file) {
...
*************************************************/
int VOSConfig::getConfigInt(const string& s_section, const string& s_item)
{
...
@Others:        svr运行时候获取，如果不能获取，证明配置有误
*************************************************/
const char * VOSConfig::getConfigStr(const string& s_section, const string& s_item)
{
   CONFIG_CONTENER::iterator iter = m_contener.find(s_section);
...
const string & VOSConfig::getCfgStr(const string& s_section, const string& s_item)
{
   CONFIG_CONTENER::iterator iter = m_contener.find(s_section);
...
//@Others:        svr运行时候获取，如果不能获取，证明配置有误
//*************************************************/
//string VOSConfig::loadConfigStr(const string& s_section, const string& s_item)
//{
//    int iRet;
//    char sResult[1024];
